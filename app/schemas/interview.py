...
class QuestionGenerateRequestDTO(BaseModel):
    categories: List[str] = Field(..., min_length=1)      # 카테고리 목록 (최소 1개) (예: ["백엔드 개발자", "운영체제", "네트워크"])
    questionCount: int = Field(..., ge=1, le=5)            # 생성할 질문 수 (1~5개)
class QuestionItemDTO(BaseModel):
    ...
    content: str         # 질문 내용
    intent: str          # 출제 의도
    keywords: List[str] = Field(..., min_length=2, max_length=4)  # 핵심 키워드 리스트
# ## 면접 질문 생성 응답 DTO
